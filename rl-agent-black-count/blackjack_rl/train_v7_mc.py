...
def main():
    ...
    print("V7 Monte Carlo — every-visit, actual returns, no bootstrapping")
    ...
    for ep in range(1, total_eps + 1):
        ...
        while not done:
            ...
            steps.append((state.copy(), action))
            state = next_state
            hand_result += reward

        agent.decay_epsilon()

        # update every visited state with the actual return from this hand
        G = hand_result
        for visited_state, taken_action in steps:
            bucket = int(np.clip(round(float(visited_state[3])), -4, 4))
            if bucket >= 1:
                agent.mc_update(visited_state, taken_action, G)
                applied += 1
            else:
                skipped += 1
    ...
    save_path = os.path.join(root_dir, "qtable_v7_mc.npy")
